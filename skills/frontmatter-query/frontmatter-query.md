...
Use `~/.claude/tools/frontmatter-query/bin/frontmatter-query` to query YAML frontmatter in markdown files. Do NOT parse frontmatter manually with grep, awk, or sed.
...
```bash
~/.claude/tools/frontmatter-query/bin/frontmatter-query list path/to/dir
~/.claude/tools/frontmatter-query/bin/frontmatter-query list path/to/file.md
```
...
```bash
~/.claude/tools/frontmatter-query/bin/frontmatter-query search path/ -k allowed-tools -v Bash
~/.claude/tools/frontmatter-query/bin/frontmatter-query search path/ -k tags -v python
```
...
```bash
~/.claude/tools/frontmatter-query/bin/frontmatter-query tags path/
~/.claude/tools/frontmatter-query/bin/frontmatter-query tags path/ -k allowed-tools
```
...
Commands using `~/.claude/tools/frontmatter-query/bin/frontmatter-query` can be auto-approved in Claude Code hooks. This is safe because the script is read-only (stdout output only, no disk writes).
